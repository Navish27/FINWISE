...
const financeReducer = (state: FinanceState, action: FinanceAction): FinanceState => {
  ...
  switch (action.type) {
    ...
    case 'MAKE_LOAN_PAYMENT':
      const loanToUpdate = state.loans.find(loan => loan.id === action.payload.loanId);
      if (loanToUpdate) {
        const newBalance = Math.max(0, loanToUpdate.currentBalance - action.payload.amount);
        newState = {
          ...state,
          ...
          transactions: [{
            ...
            description: `Loan payment - ${loanToUpdate.type} (${loanToUpdate.bank})`,
            category: 'Loans',
